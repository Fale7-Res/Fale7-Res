...
async function loadMenu() {
  ...
  try {
    ...
    pagesEl.innerHTML = data.pages.map((page, index) => `<figure class="svg-page"><img src="/api/pages/${encodeURIComponent(page.id)}/file?v=${encodeURIComponent(page.updatedAt || data.updatedAt || '')}" alt="${escapeHtml(page.name)}" loading="${index === 0 ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${index === 0 ? 'high' : 'low'}"></figure>`).join('');
  } catch {
